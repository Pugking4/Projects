    ...
    def assign_description_to_rooms(game_map):
        ...
        for i in range(len(game_map)):
            for j in range(len(game_map[i])):
                if game_map[i][j] == 0:
                    room_description = gen.generate_empty_room_description()
                    room_desc[f'[{i}, {j}]'] = room_description
                elif game_map[i][j] == 1:
                    room_description = gen.generate_treasure_room_description()
                    room_desc[f'[{i}, {j}]'] = room_description
                elif game_map[i][j] == 2:
                    monster, generate_random_monster_count = gen.generate_random_monster(generate_random_monster_count)
                    room_description, monster_id = gen.generate_monster_room_description(monster)
                    room_desc[f'[{i}, {j}]'] = room_description
                    monster_rooms[f'[{i}, {j}]'] = {'monster_id': monster_id}
                elif game_map[i][j] == 3:
                    boss = gen.generate_random_boss()
                    room_description = gen.generate_boss_room_description(boss)
                    room_desc[f'[{i}, {j}]'] = room_description
                elif game_map[i][j] == 4:
                    shopkeeper = gen.generate_random_shopkeeper()
                    item = gen.generate_random_item()
                    room_description = gen.generate_shop_room_description(shopkeeper, item)
                    room_desc[f'[{i}, {j}]'] = room_description
                elif game_map[i][j] == 5:
                    room_description = gen.generate_entrance_room_description()
                    room_desc[f'[{i}, {j}]'] = room_description
                elif game_map[i][j] == 6:
                    room_description = gen.generate_exit_room_description()
                    room_desc[f'[{i}, {j}]'] = room_description
...
class act:
    ...
    def move_player(action, player_loc, game_map, old_display_loc):
        if action == "forward" or action == "f":
            if player_loc[0] == 0:
                ...
                act.print_map(game_map, player_loc)
    ...
    def test_actions(player_loc, game_map, room_desc, bank_int, old_display_loc, monster_out, player):
        act.action_event(player_loc, game_map, room_desc, 'f', bank_int, old_display_loc, monster_out, player)
        act.look(player_loc, room_desc)
        act.action_event(player_loc, game_map, room_desc, 'b', bank_int, old_display_loc, monster_out, player)
        act.look(player_loc, room_desc)
        act.action_event(player_loc, game_map, room_desc, 'l', bank_int, old_display_loc, monster_out, player)
        act.look(player_loc, room_desc)
        act.action_event(player_loc, game_map, room_desc, 'r', bank_int, old_display_loc, monster_out, player)
        act.look(player_loc, room_desc)
    
    def look(player_loc, room_desc):
        message = room_desc[str(player_loc)]
        print(misc.create_textbox(message))

    def action_event(player_loc, game_map, room_desc, action, bank_int, old_display_loc, monster_out, player):
        global money, bank
        if action == "help":
            help()
        elif action == "quit":
            ...
            player_loc = act.move_player(action, player_loc, game_map, old_display_loc)
        elif action == "look":
            act.look(player_loc, room_desc)
        elif action == "money":
            print(misc.create_textbox(act.print_money(money, bank, bank_int)))
        elif action == "deposit":
            money, bank = act.deposit(money, bank)
        elif action == "test":
            act.test_actions(player_loc, game_map, room_desc, bank_int, old_display_loc, monster_out, player)
        elif action == "monster":
            print(comgen.generate_random_monster_stats(monster_rooms))
        elif action == "combat":
            #try:
            com.combat(monster_out[str(player_loc)], player, monster_out, player_loc)
            #except:
                #print(misc.create_textbox("There is no monster here!"))
...
class com:
    def combat(monster, player, monster_out, player_loc):
        global admin, money, bank, bank_int, death
        monster = monster_out[str(player_loc)]
        print(misc.create_textbox(f"You have encountered a {monster['name']}!"))
        while True:
            wait = time.sleep(3)
            ...
            print(misc.create_textbox(f"Your stats: {player_stats}"))
            print(misc.create_textbox(f"{monster['name']}'s stats: {monster_stats}"))
            wait
            #print(f"{monster['name']}'s ability: {monster['ability']}")
            if player['speed'] > monster['stats']['speed']:
                print(misc.create_textbox(f"You are faster than the {monster['name']}!"))
                print(misc.create_textbox(f"What would you like to do?"))
                action = input(">>> ")
                if action == "attack":
                    print(misc.create_textbox(f"You attack the {monster['name']}!"))
                    monster['stats']['health'] -= player['attack']
                    wait
                    if monster['stats']['health'] <= 0:
                        print(misc.create_textbox(f"You have defeated the {monster['name']}!"))
                        print(misc.create_textbox(f"You have earned ${monster['stats']['health'] * 2}!"))
                        money += monster['stats']['health'] * 2
                        return money, bank, bank_int
                    print(misc.create_textbox(f"The {monster['name']} attacks you!"))
                    player['health'] -= monster['stats']['attack']
                    wait
                    if player['health'] <= 0:
                        print(misc.create_textbox(f"You have been defeated by the {monster['name']}!"))
                        death = 1
                        return death
                elif action == "stats":
                    print(misc.create_textbox(f"Your stats: {player_stats}"))
                    ...
                    if random.randint(1, 100) <= player['luck']:
                        print(misc.create_textbox(f"You run away from the {monster['name']}!"))
                        return money, bank, bank_int
                    else:
                        print(misc.create_textbox(f"You failed to run away from the {monster['name']}!"))
                        wait
                        print(misc.create_textbox(f"The {monster['name']} attacks you!"))
                        player['health'] -= monster['stats']['attack']
                        wait
                ...
                wait
                print(misc.create_textbox(f"The {monster['name']} is faster than you!"))
                print(misc.create_textbox(f"The {monster['name']} attacks you!"))
                player['health'] -= monster['stats']['attack']
                wait
                if player['health'] <= 0:
