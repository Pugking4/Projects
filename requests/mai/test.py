...
async def scrape(version, segaid, password):
    async with async_playwright() as p:
        ...
        await page.wait_for_selector(f'button[value="{3}"]')

        # click the master music button
        await page.click(f'button[value="{3}"]')

        # wait for the page to load after clicking the button
        await page.wait_for_selector('div[class="music_master_score_back pointer w_450 m_15 p_3 f_0"]')
        await page.wait_for_load_state('load')

        # parse the HTML content
        wrapper = await page.query_selector('div.wrapper.main_wrapper.t_c')
        div_elements = await wrapper.query_selector_all('div[class="music_master_score_back pointer w_450 m_15 p_3 f_0"]')
        ...
        for test in results:
            with open(f'output_{version}.txt', 'a', encoding='utf-8') as f:
                f.write(test)
                f.write('|')

        #Print a message to indicate that the program has finished executing and the output has been saved
        print(f"Saved version {version} as output_{version}.txt'")
        print(len(results))
